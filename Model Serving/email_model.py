...
dataset = pd.read_csv(r'C:/Users/gcao/model_en/emails.csv')
dataset.columns #Index(['text', 'spam'], dtype='object')
print(dataset.shape)
dataset.drop_duplicates(inplace = True)
print(dataset.shape)
print(pd.DataFrame(dataset.isnull().sum()))
dataset['text'] = dataset['text'].apply(lambda x: ' '.join(x.split(' ')[1:]))
...
def predict():
    ...
    probability = predictions[0][1][0]/(predictions[0][1][0] + predictions[0][1][1])
    print(predictions)
    handle_result = {"result": predictions[0][2], "probability": json.dumps(probability.item())}
    try:
